...
The calculator, stored-visualization decoder, browser, and cache accept only
the current v29/v23 contract with an explicitly supported pinned grid profile;
the production web writer uses `production-v2`. Older datasets are not migrated
or reinterpreted; a new successful calculation must replace an older fixture.

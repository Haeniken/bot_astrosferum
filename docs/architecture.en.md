...
`CLCL/CLCM/CLCH` tier guards with maximum-random overlap and caps them by
diagnosed cover.
...
[calculation note](research-overall-index.en.md).
...
Measured results and remaining checks are recorded in [data-source-spike.en.md](data-source-spike.en.md). ICON-EU keys/domain/volume have been sampled, ICON Global CDO remapping is validated, and ICON-Ru discovery metadata and topic are known; a complete run and one real WIS notification remain unmeasured.

...
- a thin web process with Telegram OpenID Connect, server-side sessions,
...
- a dependency-free WebGL2 inside-dome view, Canvas 2D and accessible-table
  ...
  fail-closed dataset validation; the star layer stays behind the mesh, only
  one selected-cell inspector is visible, and view/fullscreen controls clear
  the compass;
- separate Compose templates for `/opt/docker/bot-astrosferum-web`, the
...
2. Provision Telegram OIDC and the pinned Alice→Dragon origin, then complete
   the documented canary, non-regression, rollback, and browser smoke gates.

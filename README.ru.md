...
В репозитории также реализовано веб-приложение направленного атмосферного
**«Астрокупола»** для
`astrosferum.com`: до 72 последовательных нативных почасовых направленных наборов данных от явно обозначенной
внешней границы расчёта `10°` до единственного зенитного узла без азимута.
Пользователь может вращать количественный купол, выбирать ячейку и величину
либо перейти к доступной 2D-карте и таблице. Для каждого направления сначала
интерполируются только исходные поля ICON-EU, затем полностью пересчитывается
физическая модель; готовые сиинг, `tau0`, пропускание облаков, Overall и
качество данных не интерполируются. Публикуются отдельные номинальное и
консервативное пропускания облаков, а Overall использует консервативное. Это
купол модельной турбулентности, облачной преграды и наклонного водяного столба,
а не полная модель наблюдаемого неба: направленные аэрозольная/молекулярная
экстинкция, удалённые туман и осадки, фон Луны/дня, атмосферная дисперсия и
направленная искусственная засветка не входят. Веб-идентичность даёт Telegram OpenID
Connect, а точки берутся из того же хранилища. Код и шаблоны развёртывания уже
есть; доступ к расчётам контролируется, а неавторизованные посетители могут
открыть одну постоянную эталонную визуализацию только для чтения. Текущий записывающий
контракт — production-v2/v29/v23. Для явно ограниченных приближений короткого
пути он публикует пониженное качество `limited` и допускает суммарно не более
1 м приближённого пути; остальные неразрешённые случаи по-прежнему завершаются
fail-closed. Предшествующий неизменяемый baseline v28/v22 дал 9 284 доступных
узло-часа из 9 288 за 33 мин 14 с. Первое сравнение
с более мелкими сетками также выявило существенную дискретизацию на малых
высотах. До публичного rollout остаются проверки угловой сетки на нескольких
точках, многократные холодные/тёплые ресурсные и наблюдательные проверки.
...
повторить эти ресурсные тесты. Публичный
веб-процесс разворачивается отдельным Compose-проектом и не создаёт второй
model store ICON.
...
- [Runbook развёртывания астрокупола](deploy/README.md)

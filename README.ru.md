...
Этот отдельный анализ ICON-EU сравнивает восемь направлений на 72 часовых интервалах `f001..f072`. Каждое направление использует быстрый прямой луч с геометрической высотой 10°; атмосферная рефракция остаётся только в астрокуполе. HHL означает поверхность атмосферной модели, а не измеренный местный горизонт.
...
остаётся в астрокуполе. ICON HHL служит поверхностью атмосферной модели, а не фактическим
местным горизонтом. Применяется общая с Overall калибровка, и функция никогда
...
модель. Текущий записывающий контракт — production-v2/v30/v23. Подробности:

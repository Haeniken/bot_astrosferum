...
- Telegram numeric user and chat identifiers;
...
Tokens and database credentials are read from ignored runtime files or environment variables. The application should not log message bodies, bot tokens, full Telegram updates, or exact user coordinates. Daily usage aggregates are retained for 90 days by the current PostgreSQL maintenance job; saved points remain until the user deletes them or the operator removes the account data.

...
RUN CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/bot_astrosferum ./cmd/bot_astrosferum \
    && CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/bot_astrosferum_web ./cmd/bot_astrosferum_web \
...
COPY --from=builder /out/bot_astrosferum_web /usr/local/bin/bot_astrosferum_web
